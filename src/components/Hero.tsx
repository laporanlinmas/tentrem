...
export default function Hero({ onScrollToSection }: HeroProps) {
  ...
  return (
    <header className="relative w-full min-h-[100vh] flex items-center overflow-hidden font-sans" style={{ isolation: 'isolate' }}>
      ...
      {/* Tombol Kontak Darurat — absolute di dalam hero, tidak overflow ke luar */}
      <button
        onClick={() => go('kontak-darurat')}
        title="Kontak Darurat"
        className="absolute z-20 group flex flex-col items-center gap-1 cursor-pointer"
        style={{ top: 'calc(5rem + 0.5rem)', right: 'clamp(0.75rem, 4vw, 2rem)' }}
        aria-label="Kontak Darurat"
      >
        <div className="relative flex items-center justify-center">
          <span className="absolute w-10 h-10 sm:w-14 sm:h-14 rounded-full bg-red-500/20 animate-ping" style={{ animationDuration: '2s' }} />
          <svg viewBox="0 0 56 56" className="w-10 h-10 sm:w-14 sm:h-14 drop-shadow-lg group-hover:scale-110 transition-transform duration-200" fill="none">
        ...
        <span className="text-[10px] font-black text-white/80 tracking-widest uppercase group-hover:text-white transition-colors whitespace-nowrap drop-shadow">
          Darurat
        </span>
      </button>
      ...
      {/* Content */}
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-16 lg:pt-36 lg:pb-32">
        <div className="max-w-xl xl:max-w-2xl">

          {/* Live badge */}
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 shadow-sm mb-6 max-w-[calc(100%-4.5rem)] sm:max-w-none">
            <span className="flex h-2 w-2 relative shrink-0">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-400" />
            </span>
            <span className="text-[10px] sm:text-xs font-extrabold tracking-wider uppercase text-white/90 leading-snug">
