...
async function fetchWa(): Promise<WaInfo | null> {
  // 1. Direct fetch from Firestore (settings/kontak_darurat) configured in Web Admin Aduan
  try {
    ...
    if (snap.exists()) {
      ...
      if (piket && piket.aktif !== false) {
        const rawNum = piket.noWa || piket.noHp || '';
        const num = cleanWANumber(rawNum);
        if (num) {
          return {
            number: num,
            name: (piket.namaPetugas || piket.judul || 'Regu Piket Linmas').trim(),
            jadwal: (piket.jamOperasional || '24 Jam Nonstop').trim(),
            pesanTemplate: piket.pesanWaTemplate || 'Halo Petugas Piket Linmas Desa Tugurejo, saya warga ingin meminta bantuan / informasi.',
          };
        }
      }
    }
  } catch (err) {
    console.warn('[fetchWa Firestore]', err);
  }

  // 2. Fallback to API /api/wa-number
  try {
    ...
    if (res.ok) {
      ...
      if (d?.number) {
        return {
          number: cleanWANumber(d.number),
          name: d.name || 'Regu Piket Linmas',
          jadwal: d.jadwal || '24 Jam Nonstop',
          pesanTemplate: d.pesanWaTemplate || 'Halo Petugas Piket Linmas Desa Tugurejo, saya warga ingin meminta bantuan / informasi.',
        };
      }
    }
  } catch (err) {
    console.warn('[fetchWa API]', err);
  }

  // 3. Fallback default hotline Posko Tugurejo
  return {
    number: '6282313823791',
    name: 'Posko Siaga Linmas Tugurejo',
    jadwal: '24 Jam Nonstop',
    pesanTemplate: 'Halo Petugas Piket Linmas Desa Tugurejo, saya warga ingin meminta bantuan / informasi.',
  };
}
...
function WaCard({ name, waUrl, jadwal }: WaCardProps) {
  return (
    <div className="mt-3 rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50/90 to-teal-50/60 p-3.5 shadow-sm">
      <div className="flex items-center gap-2 pb-2.5 mb-2.5 border-b border-emerald-200/60">
        <div className="w-6 h-6 rounded-full bg-emerald-500/15 flex items-center justify-center text-emerald-600 shrink-0">
          <WhatsAppIcon size={14} />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-emerald-900">Petugas Piket Siaga</span>
      ...
      <div className="space-y-1.5 text-xs text-slate-700 mb-3">
        <div className="flex gap-2">
          <span className="text-slate-400 w-16 shrink-0 font-medium">Petugas</span>
          <span className="font-bold text-slate-800">{name}</span>
        </div>
        {jadwal && (
          <div className="flex gap-2">
            <span className="text-slate-400 w-16 shrink-0 font-medium">Layanan</span>
            <span className="font-semibold text-emerald-700">{jadwal}</span>
          </div>
        )}
      </div>
      <div>
        <a
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] py-2.5 px-4 text-xs font-bold text-white hover:bg-[#1ebe5d] active:scale-[0.99] transition-all shadow-sm shadow-emerald-500/20"
        >
  ...
  const onSend = async (overrideText?: string) => {
    ...
    try {
      ...
      if (/hubungi|petugas|kontak|piket|telepon|wa linmas|whatsapp petugas|nomor linmas/i.test(text) && !/apa itu|tentang/i.test(text)) {
        ...
        if (wa && wa.number) {
          const defaultMsg = 'Halo Petugas Piket Linmas Desa Tugurejo, saya warga ingin meminta bantuan / informasi.';
          const waMsg = encodeURIComponent(wa.pesanTemplate || defaultMsg);
          addMsg(
            'bot',
            `✅ **Petugas Piket Siaga Linmas**:\n• Petugas: **${wa.name}**\n• Layanan: **${wa.jadwal}**\n\nSilakan klik tombol WhatsApp di bawah untuk chat langsung dengan petugas:`,
            {
              waCard: {
                name: wa.name,
                number: wa.number,
