...
Fitur Admin: verifikasi dan tindak lanjut aduan warga, monitoring jadwal & kelompok ronda, pengelolaan personil Satlinmas, manajemen berita desa, survei IKM, manajemen inventaris, dan penerbitan laporan resmi (PDF/Excel/DOCX).
...
Q: Apa fungsi halaman Jadwal Ronda?
...
const SYNONYMS: Record<string, string[]> = {
  ...
  danpok: ['komandan kelompok', 'komandan ronda', 'pimpinan ronda'],
  ...
  jadwal: ['jadwal ronda', 'giliran', 'piket', 'shift'],
};
...
export async function askChatbot(userQuery: string, context?: ConversationContext): Promise<string> {
  ...
  if (intentResult.primary === 'GREETING') {
    return 'Halo! 👋 Selamat datang di **TENTREM** — *Tugurejo Nyaman Tanggap Responsif Modern*, Desa Tugurejo, Kec. Slahung, Kab. Ponorogo.\n\nAda yang bisa saya bantu? Saya siap memandu Anda mengenai layanan pengaduan warga, jadwal ronda, berita desa, peta wilayah, cuaca BMKG, atau informasi Satlinmas.';
  }
  ...
  if (intentResult.primary === 'COST') {
    return 'Seluruh layanan sistem TENTREM Desa Tugurejo — pengaduan warga, jadwal ronda, berita desa, survei IKM, prakiraan cuaca BMKG, galeri, peta, hingga isyarat kentongan — semuanya **100% GRATIS** tanpa dipungut biaya apapun untuk warga masyarakat.';
  }
  ...
  // ── 5. Ronda & Poskamling ─────────────────────────────────────────────────
  if (/ronda|siskamling|poskamling|danpok|komandan kelompok|patroli malam|jadwal ronda|kelompok ronda/i.test(normalized)) {
    return '🛡️ **Jadwal Ronda TENTREM** — Sistem ronda bergilir Desa Tugurejo:\n• Jadwal kelompok ronda bergilir setiap malam\n• Waktu jaga: 21:30 – 02:00 WIB (4,5 jam)\n• Danpok (Komandan Kelompok) memimpin setiap regu\n• Jadwal lengkap tersedia di halaman Jadwal Ronda (/jadwal-ronda)\n• Dikelola oleh Satlinmas Desa Tugurejo bersama warga';
  }
  ...
  // ── 20. Jadwal Ronda ─────────────────────────────────────────────────────
  if (/jadwal ronda|giliran ronda|shift ronda|jadwal piket ronda/i.test(normalized)) {
    return 'Halaman **Jadwal Ronda** (/jadwal-ronda) menampilkan jadwal ronda malam harian secara digital, mencakup: kelompok ronda bertugas hari ini, nama-nama anggota yang bertugas, dan status Danpok aktif. Sistem ini membantu transparansi dan akuntabilitas kegiatan ronda.';
  }
